...
class GeoPoliticalFieldTestCase(TestCase):
    ...
    def test_geo_political_zone_initialization_with_data(self):
        ...
        self.assertEqual(sorted(field.get_zones()), sorted(queryset_to_list(default_zone, "name")))
    ...
    def test_geo_political_zone_initialization_with_data_and_kwargs(self):
        ...
        self.assertListEqual(sorted(field.zones), field.get_zones())
    ...
    def test_geo_political_zone_default_overriding(self):
        """
        Test that the zones kwargs override the settings.DEFAULT_GEO_POLITICAL_ZONES
        """
        load_fixtures()
        field = GeoPoliticalZoneField(zones=['North West', 'South East'])
        default = getattr(settings, 'DEFAULT_GEO_POLITICAL_ZONES', [])
        self.assertNotEqual(default, field.zones)
        self.assertEqual(sorted(['North West', 'South East']), sorted([item[0] for item in field.choices[1:]]))
...
class LocalGovernmentFieldTestCases(TestCase):
    ...
    def test_local_government_field_with_kwargs_and_data(self):
        load_fixtures()
        with override_settings(DEFAULT_GEO_POLITICAL_ZONES=[]):
            field = LocalGovernmentField(label='Local Governments',
            ...
            self.assertListEqual(sorted(field.get_zones()), sorted(PoliticalZones.values))
